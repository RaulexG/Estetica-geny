...
function ReagendarCita() {
  ...
  useEffect(() => {
    ...
    const fetchAppointmentData = async () => {
      try {
        ...
        setFormData({
          clientId: appointment.client.idClient,
          employeeId: appointment.employee.idEmployee,
          date: appointment.dateAppointment,
          hourId: appointment.hourAppointment.idHour,
          selectedServices: appointment.details.map((detail) => ({ id: detail.idService })),
          status: appointment.status,
          total: appointment.total,
        });
      } catch (error) {
        console.error('Error al cargar la cita:', error);
        setError('Error al cargar la cita. Por favor, inténtalo más tarde.');
      }
    };

    // Cargar clientes, estilistas, horarios y servicios
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/worker/citas');
    } catch (error) {
      console.error('Error al reagendar la cita:', error.response?.data || error.message);
      setError('Error al reagendar la cita. Por favor, inténtalo de nuevo.');
    }
  };
